...
import { ShoppingCart, X, Phone, Instagram, MapPin, Plus, Minus, Send } from 'lucide-react';
...
function SiteContent() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans">
      ...
      <nav className="fixed w-full z-50 bg-[#0a0a0a]/90 backdrop-blur-sm border-b border-red-900/30">
        <div className="max-w-7xl mx-auto px-4 h-20 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <img src={config.logo || "/logo.jpg"} alt="Logo" className="h-14 w-14 object-contain" />
          ...
          <div className="flex items-center gap-6">
            ...
            >
              ...
              {cart.length > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full ring-2 ring-[#0a0a0a]">
      ...
      <section
        className="relative h-[90vh] flex items-center justify-center overflow-hidden bg-cover bg-top bg-fixed bg-[length:120%]"
      ...
      >
        ...
        <div className="relative z-6 text-center px-4 max-w-4xl">
          ...
          >
            <img src={config.logo || "/logo.jpg"} alt="Logo" className="mx-auto w-40 h-40 mb-8 drop-shadow-2xl" />
        ...
        {/* Dragon Illustration on Background (Mockup) */}
        <div className="absolute -bottom-20 -left-20 w-[600px] opacity-10 pointer-events-none rotate-12">
          <img src="/dragon.jpg" alt="Dragon" className="w-full" />
        </div>
      </section>

      {/* Menu Section */}
      <section id="menu" className="py-34 px-4 relative bg-[url('/interior.jpg')] bg-cover bg-top bg-fixed bg-[length:120%]">
        ...
        <div className="relative max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
            ...
            <div className="flex flex-wrap gap-4 overflow-x-auto pb-4 md:pb-0 scrollbar-hide">
              {categories.map(cat => (
                <button
                  key={cat}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <AnimatePresence mode="wait">
              {MENU_ITEMS.filter(i => !i.disabled && i.category === activeCategory).map((item) => (
                <motion.div
                  key={item.id}
                  layout
                  ...
                  className="bg-[#151515] rounded-3xl overflow-hidden border border-white/5 hover:border-red-900/50 transition-all group"
                >
                  ...
                  <div className="p-6">
                    <h4 className="text-xl font-bold mb-2 group-hover:text-red-500 transition-colors">{item.name}</h4>
                    <p className="text-gray-400 text-sm mb-6 leading-relaxed">
                      {item.description}
                    </p>
                    <button
                      onClick={() => addToCart(item)}
                      className="w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-red-600 text-white py-3 rounded-xl transition-all font-bold uppercase tracking-widest text-xs"
                    >
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="md:w-1/2">
            <img src={config.bannerChefImage || "/chef.jpg"} alt="Chef" className="max-w-sm mx-auto drop-shadow-2xl" />
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            ...
            <div className="grid grid-cols-2 gap-4">
              <img src={config.aboutImage1 || "/dumpling2.jpg"} alt="Dumpling" className="rounded-3xl w-full h-64 object-cover mt-8" />
              <img src={config.aboutImage2 || "/dumpling3.jpg"} alt="Dumpling" className="rounded-3xl w-full h-64 object-cover" />
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Footer */}
      <footer id="contact" className="bg-black py-20 px-4 border-t border-white/5">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-16">
          <div>
            <div className="flex items-center gap-2 mb-8">
              <img src={config.logo || "/logo.jpg"} alt="Logo" className="h-12 w-12" />
